-- Migration 033: Include team logos in standings RPC results.
--
-- The return table shape changes, so drop and recreate the function instead of
...
comment on function public.get_group_standings(uuid) is
  'Returns standings for a single group, including team logo URLs. All assigned teams are shown with 0s before matches are played. Only counts completed, affects_standings=true matches.';
